...
def remove_once(source: str, target: str) -> str:
...
def main() -> int:
    ...
    for import_line in DIRECT_IMPORTS_TO_REMOVE:
        new_source = remove_once(new_source, import_line)

    for block in OPTIONAL_IMPORT_BLOCKS:
        new_source = remove_once(new_source, block)

    # Importante: validamos después de quitar los bloques opcionales.
    # Si validamos antes, los alias importados dentro de esos bloques producen falsos positivos.
    remaining_names = collect_names(new_source)
    still_used = sorted(NAMES_EXPECTED_ABSENT & remaining_names)
    if still_used:
        raise RuntimeError(
    ...
    missing_present = sorted(NAMES_EXPECTED_PRESENT - remaining_names)
    if missing_present:
        raise RuntimeError(
    ...
    if new_source == source:
        print("Sin cambios: Sprint 2R ya parece aplicado.")
        return 0
