...
HELPERS_PATH = Path("modulos/app_analysis_helpers.py")
...
IMPORT_ANCHOR = "from modulos.app_integrations import inyectar_atajo_teclado, load_lottieurl, obtener_modelo_gemini, obtener_secreto_streamlit\n"
IMPORT_LINE = "from modulos.app_analysis_helpers import analizar_sentimiento_noticias, escanear_vulnerabilidades, ultimo_ratio\n"

OLD_UTILS_IMPORT = "from modulos.utils import cargar_datos, calcular_score_buffett, analizar_sentimiento_noticias as analizar_sentimiento_noticias_utils\n"
NEW_UTILS_IMPORT = "from modulos.utils import cargar_datos, calcular_score_buffett\n"
...
def main() -> int:
    ...
    if "st.set_page_config(" not in source[:400]:
        raise RuntimeError("st.set_page_config no parece estar al inicio. Revisa app.py antes de aplicar Sprint 2P.")

    if IMPORT_LINE in source and HELPERS_PATH.exists():
        print("Sin cambios: Sprint 2P ya parece aplicado.")
        return 0
    ...
    required_tokens = [
        ...
        "def ultimo_ratio(",
        "analizar_sentimiento_noticias_utils(ticker)",
        "Deuda / Capital",
    ...
    if IMPORT_LINE not in new_source:
        if IMPORT_ANCHOR not in new_source:
            raise RuntimeError("No se encontró el import de app_integrations para insertar app_analysis_helpers.")
        new_source = new_source.replace(IMPORT_ANCHOR, IMPORT_ANCHOR + IMPORT_LINE, 1)

    if OLD_UTILS_IMPORT in new_source:
        new_source = new_source.replace(OLD_UTILS_IMPORT, NEW_UTILS_IMPORT, 1)

    backup = APP_PATH.with_suffix(".py.bak_sprint_2p")
    ...
    print("OK: Sprint 2P aplicado.")
    print(f"Helpers de análisis extraídos a: {HELPERS_PATH}")
    print(f"Backup creado: {backup}")
    print("Valida con: python -m py_compile app.py modulos/app_analysis_helpers.py")
    return 0
